...
#  Fabric8 Maven Plugin modifications

- Change the version from 3.2.9 to 3.1.92
- Add the generator wildfly-swarm that we will use
...
- Deploy on Openshift `fabric8:deploy`
...
- Add a service.yml under src/main/fabric8 where the target port is 8081
[source]
----
apiVersion: v1
kind: Service
...
[source]
----
mvn fabric8:deploy fabric8:undeploy
----
