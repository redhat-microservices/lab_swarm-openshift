...
. Add WildFly Swarm Hystrix dependency to the pom.xml of the `cdservice`
+
[source]
----
<dependency>
...
. Add Hystrix enabled label to the service definition (svc.yml)
+
[source]
----
metadata:
...
public class GetCatalogListCommand extends HystrixCommand<List> {
    /* class attributes and constructor were ommitted for brevity */
...
. DESCRIBE HOW WE WILL ACCHIEVE TO THIS RESULT
+
[source]
----
public class GetCatalogListCommand extends HystrixCommand<List> {
...
. Use GetCatalogListCommand whenever catalog list is required within the `CatalogEndpoint.class`
+
[source]
----
public List<Catalog> listAll(@QueryParam("start") Integer startPosition,
			@QueryParam("max") Integer maxResult) {
   new GetCatalogListCommand(entityManager, startPosition, maxResult).execute();
}
----

. Redeploy the service modified
+
[source]
----
