...
. To be able to play with the project locally, add a folder `src/main/config` containing a `project-stages.yaml` file
. Configure the datasource to use H2 in-memory database with `ExampleDS` as datasource name
+
[source]
----
swarm:
...
. Define a profile within the `pom.xml` file where we will tell to maven to copy the `src/main/config` content to the target folder `src/main/resources`
  Specify alo the h2 database dependency
+
[source]
----
 <profile>
...
. Move the `persistence.xml` file from the src/main/resources directory to the directory `src/main/config-openshift/META-INF`
...
. Crate the MySQL application using the OpenShift MySQL Template
+
[source]
----
...
. Deploy on OpenShift using the cd front and the cd service using the `fabric8:deploy` maven goal
