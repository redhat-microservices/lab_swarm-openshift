...
. Change the `MY_HOST_IP_ADDRESS` key with the value of the private IP address of your `cdservice`
...
. Add WildFly Swarm Hystrix dependency to the pom.xml of the `cdservice` (pom.xml)
+
[source]
----
<dependency>
...
. Add Hystrix enabled label to the service definition (src/main/fabric8/svc.yml)
+
[source]
----
metadata:
...
. Use GetCatalogListCommand to get a catalog list withing the
Use GetCatalogListCommand whenever catalog list is required within the src/main/java/org/cdservice/rest/CatalogEndpoint.java
+
[source]
----
